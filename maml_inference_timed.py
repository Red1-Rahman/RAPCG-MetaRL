...
class MAMLPolicyWrapper:
    def __init__(self, policy: MAMLPolicy, device: str,
                 obs_dim: int, adapted_params: Optional[OrderedDict] = None):
        self.policy         = policy
        self.device         = device
    ...
    def predict(self, obs: np.ndarray,
        ...
        if feat < self.obs_dim:
            pad = torch.zeros(obs_flat.shape[0], self.obs_dim - feat,
                              device=self.device)
        ...
        with torch.no_grad():
            if self.adapted_params is not None:
                logits, _ = self.policy.functional_forward(obs_flat,
                                                           self.adapted_params)
            else:
...
def load_maml_checkpoint(checkpoint_path: str, obs_dim: int,
...
def build_env(game: str, representation: str,
              resource_monitor: ResourceMonitor) -> DummyVecEnv:
    def _make():
        env = make_pcgrl_env(
...
def get_env_dims(env: DummyVecEnv) -> Tuple[int, int]:
    ...
    action_dim = act_space.n if hasattr(act_space, 'n') \
...
def adapt_policy(policy: MAMLPolicy, env: DummyVecEnv,
                 inner_lr: float, inner_steps: int,
                 n_trajectories: int, device: str) -> OrderedDict:
    ...
    for k in range(inner_steps):
        traj  = collect_trajectories(env, policy, n_trajectories,
                                     device, adapted)
...
_CSV_COLUMNS = [
    'level_id', 'timestamp', 'game', 'algorithm', 'adapt_steps',
    'reset_time_ms', 'generation_time_ms', 'extract_time_ms',
    'validation_time_ms', 'metrics_time_ms', 'solvability_time_ms',
    'save_time_ms', 'total_time_ms',
    'steps', 'mean_inference_ms', 'std_inference_ms',
    'min_inference_ms', 'max_inference_ms',
    'total_reward', 'diversity', 'complexity', 'unique_tiles', 'is_solvable',
    'ram_start_pct', 'ram_end_pct', 'ram_delta_pct',
    'cpu_start_pct', 'cpu_end_pct', 'cpu_delta_pct',
    'gpu_start_pct', 'gpu_end_pct', 'gpu_delta_pct',
]
...
class LiveCSVWriter:
    ...
    def __init__(self, path: str):
        ...
        self._w   = csv.DictWriter(self._fh, fieldnames=_CSV_COLUMNS,
                                   extrasaction='ignore')
        self._w.writeheader()
        self._fh.flush()
...
def generate_timed(
    ...
    # ---- Load checkpoint -------------------------------------------------
    policy, config = load_maml_checkpoint(
        checkpoint_path, obs_dim, act_dim, device)
    ...
    for i in range(n_levels):
        ...
        try:
            save_level_image(level, lpath + '.png', game=game,
                             scale=25, show_grid=True, dpi=300)
        except Exception as e:
            print(f"  [WARN] Image save failed: {e}")
        save_time = time.perf_counter() - save_start

        total_time = (reset_time + gen_time + extract_time +
                      validation_time + metrics_time +
                      solvability_time + save_time)
        ...
        print(f"  CPU={res_end['cpu_percent']:.0f}%  "
...
def main():
    parser = argparse.ArgumentParser(
        description='Timed MAML inference – paper metrics')
    ...
    parser.add_argument('--log-file',        default='inference_timing_maml.csv',
                        help='Per-level timing CSV (row flushed after every level)')
    parser.add_argument('--log-dir',         default='logs',
                        help='Directory for TrainingLogger step-level CSV')
