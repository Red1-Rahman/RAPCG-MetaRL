        ...
        for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if (ny, nx) in crate_set:
                ...
                if not (0 <= next_box_y < h and 0 <= next_box_x < w):
                    continue
                if (
                    walls[next_box_y, next_box_x]
                    or (next_box_y, next_box_x) in crate_set
                ):
                    continue
                ...
                new_crates = tuple(
                    sorted(
                        [
                            (next_box_y, next_box_x) if c == (ny, nx) else c
                            for c in crates
                        ]
                    )
                )
...
def fix_level(raw):
    ...
    if int(np.sum(level == 3)) == 0:
        ...
        if empties:
            level[empties[0]] = 3
            issues.append("added missing crate")
    if int(np.sum(level == 4)) == 0:
        empties = list(zip(*np.where(level == 0)))
        if empties:
            level[empties[0]] = 4
            issues.append("added missing target")
    ...
    while not solve_sokoban(level) and attempts < 15:
        ...
        valid_crate_spots = [
            t for t in live_squares if t in reachable_tiles and level[t] == 0
        ]
        ...
        if current_crates and valid_crate_spots:
            ...
            level[new_spot] = 3
            issues.append(
                f"unsolvable state fallback: moved crate {bad_crate} to {new_spot}"
            )
        else:
...
for i, pref in enumerate(prefs):
    ...
    if issues_a or issues_b:
        ...
        entry_issues = {}
        if issues_a:
            entry_issues["level_a"] = issues_a
        if issues_b:
            entry_issues["level_b"] = issues_b
...
for item in all_issues:
    ...
    if "level_a" in item:
        print(f"A={item['level_a']}", end="  ")
    if "level_b" in item:
        print(f"B={item['level_b']}", end="")
    print()
