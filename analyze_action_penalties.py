...
def analyze_action_correlations(df: pd.DataFrame, 
                                penalty_type: str = 'penalty_ram_penalty',
                                min_action_count: int = 10) -> pd.DataFrame:
    """
    ...
    action_stats.columns = ['_'.join(col).strip('_') if col[1] else col[0] 
                           for col in action_stats.columns.values]
    
    # Filter by minimum count
    action_stats = action_stats[action_stats[f'{penalty_type}_count'] >= min_action_count]
...
def print_action_analysis(action_stats: pd.DataFrame, penalty_type: str, top_n: int = 10):
    ...
    print("-"*80)
    print(f"{'Action':>8} {'Count':>8} {'Avg Penalty':>12} {'Max Penalty':>12} {'Avg RAM %':>10}")
    print("-"*80)
    ...
    for idx, row in action_stats.head(top_n).iterrows():
        ...
        print(f"{action:8d} {count:8d} {avg_penalty:12.4f} {max_penalty:12.4f} {avg_ram:10.2f}")
    ...
    print(f"  Mean penalty across all actions: {action_stats[penalty_mean_col].mean():.4f}")
    print("="*80 + "\n")


def plot_action_heatmap(df: pd.DataFrame, output_file: str = 'action_penalty_heatmap.png'):
    ...
    ax2 = axes[1]
    ax2.plot(df_sample['step'], df_sample['penalty_ram_penalty'], alpha=0.6, linewidth=0.5)
    ...
    ax3 = axes[2]
    ax3.plot(df_sample['step'], df_sample['ram_percent'], alpha=0.6, linewidth=0.5, color='red')
...
def plot_action_distribution(action_stats: pd.DataFrame, 
                            penalty_type: str = 'penalty_ram_penalty',
...
def main():
    ...
    parser.add_argument('--penalty', default='penalty_ram_penalty', 
    ...
    action_stats = analyze_action_correlations(
        ...
        min_action_count=args.min_count
    )
