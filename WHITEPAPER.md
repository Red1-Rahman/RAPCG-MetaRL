...
Sokoban is PSPACE-complete. Exact solvability verification is computationally intractable at the scale of a reinforcement learning training loop, where the agent may take millions of steps across thousands of candidate layouts. Embedding a complete solver as a per-step reward signal would dominate wall-clock training time and make the reward landscape dependent on solver budget rather than policy quality. RAPCG-MetaRL therefore does not attempt to guarantee exact solvability during training. Instead, it optimizes for structural solvability indicators: measurable, low-cost properties that strongly correlate with solvability without requiring exhaustive search. This framing is consistent with the broader PCGRL literature, which treats solvability as a target metric evaluated at inference time rather than a hard constraint enforced during every environment step.

The active Sokoban validation code is `sokoban_utils.py`. It enforces one player, balanced crate-target counts, deadlock removal, pushability checks, player reachability, crate-to-target reachability, target placement checks, and minimum crate-target pairs. These checks are individually polynomial and collectively serve as a structural solvability proxy. Exact solvability checking using the `gym-pcgrl` Sokoban engine is available as an offline evaluation tool, attempting BFS followed by A\* variants with different balance settings under a configurable solver budget. The default solver budget exposed in `solvability_config.py` is 5,000 iterations. This budget is intentionally used for post-hoc evaluation of generated level batches rather than as an inline training signal.

The Sokoban environment can be wrapped by `SokobanSolvabilityWrapper`. On reset it can validate and repair the current map. During step execution it evaluates terminal or reported maps, checks structural validity, applies the unsolvable penalty when needed, and records statistics such as total levels checked, solvable levels, unsolvable levels, and solution lengths.
...
Human-guided preference signals provide a complementary path toward structurally valid layouts. Where reward shaping encodes designer knowledge explicitly through tuned weight terms, RLHF allows that knowledge to be expressed implicitly through pairwise comparisons. When annotators consistently prefer levels with balanced crate-target counts, reachable crates, and no corner deadlocks, the learned reward model captures those structural regularities without requiring them to be hand-coded as formula terms. The RLHF fine-tuned policy is therefore expected to improve solvability proxy metrics relative to unguided PPO, not because the reward model solves Sokoban, but because human structural intuition transfers into the reward signal through the preference learning step.
