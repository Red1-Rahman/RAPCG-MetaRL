...
def analyze_forward_levels(levels: List[np.ndarray]) -> Dict:
    ...
    for i, level in enumerate(levels):
        ...
        # Apply validation
        fixed_level, corrections = validate_and_fix_sokoban(level, min_crates=1, enforce_all_rules=True)
...
def create_comparison_report(forward_stats: Dict, backward_stats: Dict, output_dir: str):
    """
    ...
    validity_data = {
        'Forward\n(before fix)': forward_stats['valid_before_fix'] / forward_stats['total_levels'] * 100,
        'Forward\n(after fix)': forward_stats['valid_after_fix'] / forward_stats['total_levels'] * 100,
        'Backward': backward_stats['valid_levels'] / backward_stats['total_levels'] * 100
    }
    bars = ax1.bar(validity_data.keys(), validity_data.values(), color=['#e74c3c', '#f39c12', '#27ae60'])
    ...
    ax3.hist([forward_stats['crate_counts'], backward_stats['crate_counts']], 
            label=['Forward', 'Backward'], bins=range(0, 8), alpha=0.7)
    ...
    ax4.boxplot([forward_stats['wall_densities'], backward_stats['wall_densities']],
    ...
    if backward_stats['solution_lengths']:
        ...
        ax5.hist(backward_stats['solution_lengths'], bins=15, color='#27ae60', alpha=0.7)
        ax5.set_xlabel('Solution Length')
        ax5.set_ylabel('Frequency')
        ax5.set_title('Backward: Solution Length Distribution')
        ax5.axvline(np.mean(backward_stats['solution_lengths']), 
                   color='red', linestyle='--', label=f"Mean: {np.mean(backward_stats['solution_lengths']):.1f}")
        ax5.legend()
    ...
    ax6.text(0.1, 0.5, summary_text, fontfamily='monospace', fontsize=9, 
    ...
    ax7.hist(correction_rates, bins=range(0, max(correction_rates)+2), color='#e74c3c', alpha=0.7)
    ...
    forward_quality = (forward_stats['valid_after_fix'] / forward_stats['total_levels']) * 100
    backward_quality = (backward_stats['valid_levels'] / backward_stats['total_levels']) * 100
    
    # Compute composite quality score
    forward_score = forward_quality - (sum(correction_rates) / len(correction_rates) * 5)
    ...
    with open(stats_path, 'w') as f:
        json.dump({
            'forward': {k: v for k, v in forward_stats.items() if k != 'correction_details'},
...
def main():
    parser = argparse.ArgumentParser(description='Compare forward vs backward generation')
    parser.add_argument('--forward-dir', type=str, required=True,
                       help='Directory with forward-generated levels')
    parser.add_argument('--backward-dir', type=str, required=True,
