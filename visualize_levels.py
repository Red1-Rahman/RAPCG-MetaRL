...
def level_to_rgb(
    level: np.ndarray, game: str = "zelda", tile_size: int = 16
) -> np.ndarray:
    """
...
def render_level(
    level: np.ndarray, game: str = "zelda", scale: int = 20, show_grid: bool = True
) -> np.ndarray:
    """
...
def create_training_comparison(
    ...
    for i in range(n):
        ...
        axes[0, i].set_title(
            f"Before Training - Sample {i + 1}", fontsize=12, fontweight="bold"
        )
        axes[0, i].axis("off")
        ...
        axes[1, i].set_title(
            f"After Training - Sample {i + 1}", fontsize=12, fontweight="bold"
        )
        axes[1, i].axis("off")
...
def visualize_from_file(
    filepath: str, game: str = "zelda", output_dir: str = "visualizations"
):
    """
...
def demo_visualizations(game: str = "zelda"):
    ...
    create_training_comparison(
        levels[:2], levels[2:4], game, save_path=comparison_path, dpi=300
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--demo", action="store_true", help="Generate demo visualizations"
    )
    parser.add_argument(
        "--file", type=str, default=None, help="Path to .npy level file to visualize"
    )
    parser.add_argument(
    ...
    if args.demo:
        ...
        print("Usage:")
        print(
            "  python visualize_levels.py --demo                    # Generate demo figures"
        )
        print(
            "  python visualize_levels.py --file level.npy --game zelda  # Visualize specific level"
        )
